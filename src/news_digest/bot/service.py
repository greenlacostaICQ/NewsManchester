...
/useful <fingerprint> — отметить пункт полезным
/notuseful <fingerprint> — отметить пункт неполезным
...
class DigestBotService:
    ...
    def _process_single_update(self, update: dict[str, Any]) -> int | None:
        ...
        if command in {"/useful", "/notuseful"}:
            return self._record_feedback(str(chat_id), text, command)
    ...
    def _unknown_command_text(self) -> str:
        return "Не понял команду. Доступно: /latest, /subscribe, /unsubscribe, /useful <fingerprint>, /notuseful <fingerprint>, /help."

    def _record_feedback(self, chat_id: str, text: str, command: str) -> int:
        parts = text.split(maxsplit=1)
        if len(parts) < 2 or not parts[1].strip():
            self.client.send_message(chat_id, "Нужен fingerprint пункта: например /useful abc123.")
            return 1
        fingerprint = parts[1].strip().split()[0]
        reaction = "useful" if command == "/useful" else "not_useful"
        ok = self.store.record_item_feedback(
            fingerprint=fingerprint,
            reaction=reaction,
            chat_id=chat_id,
        )
        if ok:
            self.client.send_message(chat_id, "Спасибо, фидбек сохранён.")
        else:
            self.client.send_message(chat_id, "Не смог сохранить фидбек: проверь fingerprint.")
        return 1
