...
_SOURCE_POLICIES: dict[str, _SourcePolicy] = {
    ...
    "Pedddle Markets": _SourcePolicy(
        path_must_contain=("/market/", "/organiser/"),
        block_food_evergreen=True,
        require_gm_token=True,
        require_food_opening_or_place=True,
    ),
    "Pedddle Makers Market": _SourcePolicy(
        path_must_contain=("/market/", "/organiser/makers-market"),
        block_food_evergreen=True,
        require_gm_token=True,
        require_food_opening_or_place=True,
    ),
    "The Makers Market": _SourcePolicy(
        path_must_contain=("/venue/", "/market/", "/events", "/upcoming"),
        path_banned_segments=("/maker/", "/trader/", "/stallholder/"),
        require_gm_token=True,
        require_food_opening_or_place=True,
    ),
    "Manchester Rocks Markets": _SourcePolicy(
        path_must_contain=("/things-to-do/",),
        block_food_evergreen=True,
        require_gm_token=True,
        require_food_opening_or_place=True,
    ),
    "ReMarket Mato": _SourcePolicy(
        path_must_contain=("/event/", "/venue/remarket"),
        require_gm_or_date=True,
        require_food_opening_or_place=True,
    ),
    "VisitSalford Markets": _SourcePolicy(
        path_must_contain=("/events/", "/whats-on/"),
        require_gm_token=True,
        require_food_opening_or_place=True,
    ),
    "SK Lowdown Markets": _SourcePolicy(
        path_must_contain=("/whats-on-stockport/",),
        require_food_opening_or_place=True,
    ),
    # ── Tech / Business ───────────────────────────────────────────────────────
