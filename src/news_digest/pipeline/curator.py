...
- Evergreen-листинги без даты: «10 лучших баров», «куда пойти на выходных» — без конкретного события сегодня/завтра.
  НО: повторяющееся событие с явной каденцией (car boot «каждое воскресенье», фермерский/makers market «третья суббота месяца», «каждую пятницу») — это НЕ evergreen. У него есть конкретная дата ближайшего проведения, она вычисляется автоматически позже. include: true, не дропай как «без даты».
...
ПЕРЕКРЫВАЮЩЕЕ ПРАВИЛО (важнее списка ДРОПАЙ, кроме дублей): если в evidence_text есть хотя бы один конкретный факт — имя человека/организации, GM-район или адрес, дата/время, сумма £, число пострадавших, исход дела/решения — НЕ дропай, ставь include: true. Дроп по «PR/awareness/evergreen/нет GM» применяй только когда такой конкретики нет ни в title, ни в summary, ни в evidence_text. Дубли тем (одна и та же история) дропай всегда — это правило их не защищает.
...
- IT/бизнес — ВКЛЮЧАЙ конкретные деловые события в GM: новый офис/штаб-квартира или переезд компании, новый стартап/запуск компании, конференция/крупное отраслевое мероприятие, раунд инвестиций/финансирование, расширение или крупная стройка/девелопмент (новые рабочие места, площадка, сумма). ДРОПАЙ внутренний PR без события: годовщины («25 лет компании»), членские мероприятия/roundtable, «comment»-колонки, награды/шортлисты, релизы версий продукта без локального эффекта.
