#!/bin/bash
# Phase 2: Deterministic publish pipeline.
# Expects candidates.json to already have draft_line fields written
# by the agent (or by a previous deterministic fallback). Runs:
#   write-digest → edit-digest → build-digest → send
#
# Usage:
