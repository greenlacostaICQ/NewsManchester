...
- Статус: внедрено локально; production safe-run ожидается после push.
...
- ПРОВЕРКА (offline): полный набор `1006/1006` тестов OK. Replay 10.08: `38 shown / 0 replaced / 0 removed`, `5` content divergences → `ship_degraded`, lead `ok`, `blank_runs_2plus=0`, technical verify OK. Изменение не меняет состав исторического replay; оно меняет только collect-source decision для следующего свежего run.
