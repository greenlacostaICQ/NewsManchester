...
- Статус: внедрено; offline tests/replay пройдены, production-proof ожидается.
...
- ПРОВЕРКА (offline): текущий `venues_tickets.jsonl` перерендерен без записи state: служебная фраза `40→0`, неверные owners в именованных дефектах `0`, исходные date/venue/link сохранены `40/40`. Полный suite: `875 tests`, `OK`. Replay 19.07 до изменения: `81 slots / 79 lines / 3 removed`; после: `74 slots / 71 shown / 4 removed`, снижение состава соответствует Food `6→3` и native-only Today; lead `ok`, max blank run `1`, blank runs 2+ `0`, boilerplate `0`. Golden replay 12/12 дней: все stages OK, lead `ok` 12/12, max blank run `1`, blank runs 2+ `0`, boilerplate endings `0`; known sent defects на golden-днях подтверждены harness.
- Где была ошибка (если не сработает): —
