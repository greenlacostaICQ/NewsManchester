...
### 0027 — Skiddle: deep-enrichment затирал alt-заголовок карточки заголовком страницы — 2026-06-30
- Статус: внедрено, ПРОВЕРЕНО CI-командой (737 тестов зелёные)
- Проблема: `test_skiddle_cards_extract_event_link_and_date` падал (pre-existing, отмечен в 0026). Карточка отдаёт богатый `<img alt="Manchester Forever at Bowlers Exhibition Centre">`, а кандидат выходил с `title="Manchester Forever"`.
- Причина (корень): не парсер. `_extract_skiddle_items` берёт alt корректно (полный заголовок). Но Skiddle помечен `ok_skiddle_card` без `structured_event_hint` → `_event_card_needs_deep_enrichment` = True (missing_core) → `_enrich_item` делает живой fetch страницы события. В этой ветке `preserve_listing_title` НЕ содержал Skiddle, поэтому строка 922 перезаписывала alt-заголовок голым `<title>` страницы («Manchester Forever», без площадки). Тест поэтому был сетезависим: офлайн (fetch падает) → ветка-исключение отдавала полный заголовок и тест проходил; онлайн → затирался. Ровно тот кейс, о котором предупреждает docstring `_TRUSTED_CARD_ENRICHMENT` («Re-enriching would … overwrite it with the page <title> … Keep the card title»).
- Решение: добавил «Skiddle Manchester» и «Skiddle Manchester Bank Holiday» в set `preserve_listing_title` (`extract.py:_enrich_item`) — как уже сделано для RNCM/HOME/Manchester's Finest и прочих deep-enriched афиш. Deep-fetch ради venue/date фактов остаётся; затирается только заголовок. Теперь заголовок сетенезависим (и success, и fail отдают alt карточки).
- Почему так (отвергли альтернативы): (а) общий `or trusted_card` вместо явных имён — отвергнуто как шире необходимого: остальные trusted-card источники, доходящие до затирания, уже в set (`_DEEP_EVENT_ENRICHMENT_SOURCES`); добавление по имени — действующая конвенция файла. (б) релакс теста — отвергнуто: alt и есть правильный богатый заголовок.
- Файлы/места: `src/news_digest/pipeline/collector/extract.py` (`_enrich_item`, set `preserve_listing_title`).
- ПРОВЕРКА: `PYTHONPATH=src python3 -m unittest discover -s tests` → 737 тестов, 0 провалов (был 1 — этот). Изоляция причины подтверждена стабом `_fetch_text`: при падении fetch заголовок и так полный + fetch реально дёргался на live-URL → онлайн он затирал. Прогнали ИМЕННО CI-командой.
