...
- ПРОВЕРКА: первая production-wave `29747700346` после child-fix: Events `216.42→54.3s`; Visit Manchester `125.7–195.2→30.4–31.0s`; старые `${Tripbuilder.Path}` и venue indexes исчезли, missing-facts `127→121`. Проверка также доказала, что новый council directory URL всё ещё даёт 403; он заменён во второй итерации, её production-proof ожидается. Локально 8 child items обработаны с `1 < peak <= 4` и сохранённым input order.
