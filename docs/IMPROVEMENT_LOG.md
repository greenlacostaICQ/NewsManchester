...
- Статус: внедрено локально; production-proof ожидается после push и следующего запуска workflow.
...
- ПРОВЕРКА (offline): production-аудит 09.08 — 2 canonical URL collision-группы в primaries, 6 resurrected cheap-дублей. Baseline replay 09.08: `53` public slots, `52` draft lines, verify FAIL на двух `html_line_duplicated`, `51` replay bullets, lead ok. После изменения: `49` public slots, `48` draft lines, verify OK — `47 shown / 1 replaced / 1 removed`, `6` content divergences → `ship_degraded`, `47` bullets, lead ok; выпуск дошёл бы до send. 34 planner-contract теста и 3 связанные cheap-dedup/repeat регрессии — OK. `replay_day.py --golden`: 12/12 дней OK, technical verify failures 0, lead ok 12/12, новых `blank_runs_2plus` нет.
