...
- Статус: внедрено; offline tests/replay пройдены, ticket night production-run пройден; финальный rerun owner-нормализации ожидается после push.
...
- ПРОВЕРКА (production, предварительная): GitHub Tests run `29823942379` прошёл на `6a349a2`. Night tickets run `29823962079`: `15` источников, `815` сохранённых карточек (`816` найдено до merge), `87` morning-eligible, `0` source errors, `29.3s`, state push с первой попытки, Telegram status `200`; в опубликованном inventory служебных ticket-фраз `0`, owner заполнен `815/815`, date `804/815`, venue `803/815`, action link `815/815`. Этот run выявил единственный ложный lineup из имени площадки; исправление выше требует финального production rerun.
- Где была ошибка (если не сработает): —
