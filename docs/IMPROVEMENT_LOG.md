...
- Статус: внедрено; offline-проверка завершена, production-proof ожидается в первом `rank-digest` с `YOUTUBE_API_KEY`.
