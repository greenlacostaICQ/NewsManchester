...
- Статус: внедрено; второй production-прогон выявил смысловой дефект текста, исправление ожидает повторной night-wave.
...
- Решение: сначала используется существующий deterministic writer; строка проходит существующие strip/quality checks. Fact-complete карточки без безопасной строки ограничиваются существующими block caps и ночью идут через тот же category prompt/provider fallback, что утром. Для ночного вызова существующий prompt дополнен правилами: учитывать TODAY_DATE, не описывать прошедшую дату в будущем времени, убрать рекламные оценки и завершать конкретным действием. Результат сохраняется только после обычного `_draft_line_quality_errors`; evidence hash, prompt version и schema version проверяются при intake, изменившаяся карточка возвращается в `needs_text`.
...
- ПРОВЕРКА: реальные inventory records 13.07 — старые `old_render_ready`: Weekend 75, Ticket 29, Food 7; после текущего deterministic quality contract: 1/0/0. Production wave `20260713T124932+0100`: 15 источников, 75 найдено, 0 source errors, 13 fact-ready, 10 morning-eligible; model prewrite 8 requested / 3 written / 5 rejected. Ручная проверка трёх принятых строк нашла дефект: Food 10.07 был написан 13.07 в будущем времени, две diaspora-строки были рекламными. Это не засчитано как готовый production proof; prompt исправлен, требуется повторная волна.
- Удалено: deterministic prewrite для Food и diaspora, который в предыдущей production wave создавал шаблонные строки; отдельный новый текстовый fallback не добавлялся.
