...
- ПРОВЕРКА (offline/replay): live-shaped probes подтвердили все пять исходов; affected tests `201/201`, полный suite `1036/1036` OK (включая реальный contract-test `DeepSeek 402 → next route`). Replay 14.08 до исправления: plan `66`, `63 shown / 1 replaced / 2 removed`; после: plan `63`, `63 shown / 0 replaced / 0 removed`. Число видимых карточек сохранено, три ложных/неисполняемых слота исключены до исполнения, prose defects `0`, technical verify OK. Golden replay `12/12` завершён без новых падений.
