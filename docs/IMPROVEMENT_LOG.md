...
- ПРОВЕРКА: первая production-wave `29747700346` после child-fix: Events `216.42→54.3s`; Visit Manchester `125.7–195.2→30.4–31.0s`; старые `${Tripbuilder.Path}` и venue indexes исчезли, missing-facts `127→121`. Она же доказала, что второй council URL New Smithfield всё ещё даёт 403. После замены URL и полного seasonal-index фильтра production run `29748068447`: `health=success`, 58/58 sources checked, `source_errors=0`, 163 found, 56 fact-ready, 42 render-ready, `duration=56.39s`; New Smithfield дал 1 карточку за 0.42s, Visit sources заняли 0.46–1.32s, Visit missing-facts=0, общий missing-facts `121→115`. Локальный bound: `1 < peak <= 4`, input order сохранён.
- Удалено: оба проверенных New Smithfield council URL, возвращавших 403 на runner; последовательный Visit Manchester child-fetch path. Armed Forces запись не удалена из registry, потому что это ежегодный официальный слот, но физически исключена из активного обхода через `enabled=false`.
...
- ПРОВЕРКА: локально до последнего source-contract теста `859 tests`, `OK`, включая прежние 4 падения. Main CI `29747539017` прошёл после первого code push; окончательный main CI `29748062234` — `860 tests in 25.982s`, `OK`. State-only Events commits `8faa8fa` и `1214dbd` новых Tests runs не создали.
