...
- ПРОВЕРКА: одинаковые факты при двух fingerprint → hash equal; изменение venue → hash differs; completed court example → `(False, finished_action)`; профильный набор 188/188, полный 1030/1030. Production dry-run `31788557384`: CV contract `5 eligible → 5 sent`, outcomes `go=1 / consider=1 / skip=3`, `outcomes_conserved=true`, route failures `[]`. `reused_night_verdict=0`, потому что текущая ночная запись была создана старым кодом до смены hash; reuse остаётся production-pending до первой новой `pro_food_russian` волны.
- Где была ошибка (если не сработало): —
