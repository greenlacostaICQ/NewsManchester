...
- ПРОВЕРКА (offline): regression подтверждает прямую вставку alive operational card только при `mode=on + source_replacement_categories`; тот же вызов в `assist` остаётся `not_live_confirmed`. Inventory tests `78/78`; полный suite `1020/1020` OK. Replay 10.08: `38 shown / 0 replaced / 0 removed`, technical verify OK, lead `ok`, `blank_runs_2plus=0`.
