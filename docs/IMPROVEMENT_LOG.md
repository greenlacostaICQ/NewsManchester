...
- Статус: внедрено и подтверждено production recovery run `29988766337`.
...
### 0147 — Ранжирование отделено от карточки фактов: судья судит блок, а не запись — 2026-07-23
- Статус: внедрено; production numbers ожидаются на следующем штатном daily run.
- Проблема: `english_cards` одним вызовом делал карточку факта и ставил оценку 0-100. 23.07 из 87 участников доски 70 пришли из кэша карточек, а кэш вердикт не хранит — рейтинг отсутствовал у 80% доски. У оставшихся 24 шкала была перекошена: закрытие автобусной остановки `100`, суд над другом напавшего на синагогу `76`.
- Причина (корень): две несовместимые работы в одном вызове. Карточка — про одну новость, ответ постоянен и кэшируется; ранг — про сегодняшнюю конкуренцию, назавтра другой. Единая абсолютная шкала на разнородных рубриках заставляет модель мерить полноту записи вместо важности для читателя.
- Решение: новый модуль `board_rank.py` — один listwise-вызов на блок, без кэша, ранг 1..N внутри блока. Карточник остаётся с кэшем и больше не судит.
- Почему так: сравнение внутри блока делает перекос невозможным по построению — транспорт и суд не встречаются в одном вызове. Разделение снимает и проблему кэша: ранг больше не наследует кэш карточки.
- Ожидаемый эффект и метрика: `board_rank_report.json` — покрытие рангами по блокам и top-3 с объяснением; ~5 вызовов на блок вместо ~21 батча по 6.
- Файлы/места: `board_rank.py` (новый), `llm_rewrite.py:ENGLISH_CARD_SYSTEM/_parse_english_card_results/_apply_english_cards_to_candidates`, `model_routing.py:board_rank`.
- ПРОВЕРКА: на реальном пуле 23.07 (1237 кандидатов) разделение блоков дало 194 к судье и 1043 детерминированных; `LLM_PROVIDER=none` -> `enabled=False`, вердиктов 0, тихая деградация; `unittest discover` 909/910.
- Удалено: из промпта и парсера карточки — `editorial_score`, `selection_hint`, `board_decision`, `board_confidence`, `reason_codes`; поля `english_editorial_score`, `english_selection_hint`, `english_board_decision`, `english_board_confidence`, `english_board_reason_codes`, `english_needs_gpt4o_escalation` (копия; сырое поле эскалации сохранено). `suggested_block` удалён совсем — роутинг детерминированный, мнение модели создавало конфликт.

### 0148 — Судят только те блоки, где ответа нет в полях записи — 2026-07-23
- Статус: внедрено.
- Проблема: доска тратилась на блоки, где порядок уже определён объективными данными, и одновременно недодавала recall новостным блокам: `last_24h` видел 18 кандидатов из 88, `city_watch` 15 из 74.
- Причина (корень): доска раздавалась по всем блокам без вопроса «может ли модель добавить знание, которого нет в записи».
- Решение: `JUDGED_BLOCKS` = `lead_story`, `today_focus`, `last_24h`, `city_watch`, `tech_business`. Лимиты judged-блоков подняты: `last_24h` 18->40, `city_watch` 15->30, `tech_business` 10->15, `today_focus` 8->10, `lead_story` 4->6.
- Почему так: известность артиста лежит в Wikidata, дата события — в `event.date_start`, участок трамвая — в алерте оператора. Билеты и повторяющиеся события и так минуют модель через `_uses_deterministic_writer`; `professional_events` уже судится матчером по CV, второй голос был бы дублированием.
- Ожидаемый эффект и метрика: те же ~90 мест доски, но новостные блоки видят реальную конкуренцию; `board_rank_report.blocks` показывает охват по каждому.
- Файлы/места: `board_rank.py:JUDGED_BLOCKS`, `llm_rewrite.py:REWRITE_SHORTLIST_CAPS_BY_BLOCK`.
- ПРОВЕРКА: реальный пул 23.07 разделился 194 / 1043 ровно по заявленному списку; `test_board_rank` фиксирует, что билеты, выходные и транспорт к судье не идут.
- Удалено: ничего; несудейские блоки сохраняют прежний детерминированный порядок.

### 0149 — Судья судит ДО отсечки, а не после неё — 2026-07-23
- Статус: внедрено.
- Проблема: `_apply_rewrite_shortlist` резал список на строке 3919, а модель вызывалась на 3923. Судья получал уже урезанный пул и не мог вернуть то, что выбросила формула. Потери «Outside board» были на 100% формульными, вопреки тому, как читался отчёт.
- Причина (корень): доска исторически задумывалась как удешевление вызова, поэтому отсечка стояла перед моделью.
- Решение: `rank_boards` вызывается сразу после cost-guard и до шортлиста; `_rewrite_shortlist_priority` использует ранг судьи вместо мёртвой ветки.
- Почему так: вклад судьи симметричен около нуля (-25..+25) и равен нулю для неосуждённых, поэтому детерминированный блок не проседает просто за отсутствие судьи — старый сырой терм 0-100 делал именно это.
- Ожидаемый эффект и метрика: в judged-блоках отсечка идёт по порядку судьи; `rewrite_shortlist.board_overflow` объясняет остаток.
- Файлы/места: `llm_rewrite.py:run_rank_digest/_rewrite_shortlist_priority`, `writer.py:_section_priority_score`.
- ПРОВЕРКА: на реальном `city_watch` (74 кандидата, лимит 30) порядок незащищённых оказался строго монотонным по рангу судьи.
- Удалено: терм `(english_editorial_score - 50)/4` в `writer.py`; ветка `english_editorial_score`/`english_board_decision` в `_rewrite_shortlist_priority`; мёртвая константа `REWRITE_TRANSLATION_BOARD_MAX` (не делала ни одной отсечки, только печаталась в отчёт).

### 0150 — Вердикт reject исполняется, но никогда не в одиночку — 2026-07-23
- Статус: внедрено.
- Проблема: `board_decision` не исполнялся вообще. 23.07 из четырёх `reject` три сохранили `include=True`, один дошёл до слота `city_watch-02`. Мы платили за мнение и выбрасывали его.
- Причина (корень): единственный читатель поля работал до вызова модели, где поля ещё нет.
- Решение: reject исполняется в `_apply_rewrite_shortlist` — единственном месте, которое видит, сколько выживших осталось в блоке. Три защиты: защищённая полоса перекрывает вердикт; уверенность ниже `0.65` переводит в резерв вместо удаления; пол блока останавливает отсев. Худший ранг выбывает первым.
- Почему так: модель не должна в одиночку убивать транспорт, безопасность и решения совета, а выпуск не должен пустеть на суровое утро.
- Ожидаемый эффект и метрика: `rewrite_shortlist.board_rejects_executed` / `board_rejects_blocked` с разбивкой по причине блокировки.
- Файлы/места: `board_rank.py:board_reject_verdict`, `llm_rewrite.py:_apply_rewrite_shortlist`.
- ПРОВЕРКА: на реальном `city_watch` reject незащищённого кандидата дал `board_rejected` и `include=False`; на реальном `last_24h` все три reject были заблокированы защищённой полосой — исполнено `0`.
- Удалено: ничего; это подключение поля, которое модель и так возвращала.

### 0151 — Заглушка перестала называться судьёй — 2026-07-23
- Статус: внедрено.
- Проблема: детерминированная заглушка писалась в отчёты и дневной архив под именем `english_judge`. Строка «судья отклонил N новостей» означала отказ формулы, и отличить её от вердикта модели было нельзя.
- Причина (корень): заглушка писалась «до включения сравнения моделей», сравнение так и не включили, а имя осталось.
- Решение: `english_judge_stub` -> `formula_judge_stub`, поле `english_judge` -> `formula_judge`; на каждом кандидате появляется `judged_by` = `model` / `formula`. Guard не даёт поздней story-intelligence перетереть `model` обратно на `formula`.
- Почему так: заглушка нужна как честный запасной путь при недоступности модели — удалять её нельзя, но и выдавать за судью тоже.
- Ожидаемый эффект и метрика: по `judged_by` видно, кто решал каждую новость.
- Файлы/места: `story_intelligence.py`, `release.py`, `history.py`, `candidate_validator.py`.
- ПРОВЕРКА: на реальном кандидате из прогона 23.07 поля вышли как `formula_judge` + `judged_by: formula`; `unittest discover` 909/910.
- Удалено: имя `english_judge` во всех читателях (переименование, логика заглушки сохранена).

### 0152 — Защита «категория + tier» больше не выключает лимит в судейских блоках — 2026-07-23
- Статус: внедрено; измерено на реальном пуле 23.07.
- Проблема: судья получил контроль над отсечкой (0149), но реально решать ему было нечего: в `last_24h` 82 из 99 кандидатов и в `city_watch` 38 из 74 выбирались ВНЕ лимита блока. Все три `reject` в `last_24h` упирались в защиту, исполнялось `0`.
- Причина (корень): ветка `category in {media_layer, council, gmp, public_services, city_news} and tier in {must_include, strong}` в `_must_translate_before_cap`. Замер 23.07: из 82 защищённых в `last_24h` 79 держались только на `media_layer/strong` — то есть на состоянии, которое для новости является ДЕФОЛТНЫМ, а не исключением. Настоящих защищённых полос в блоке было 16.
- Решение: в блоках из `JUDGED_BLOCKS` эта ветка больше не даёт освобождения от лимита. Остаются `protected_lane.protected`, `today_practical_translation_reserve`, лид и блочная защита `transport`/`today_focus`. Лимиты подняты под возросшую конкуренцию: `last_24h` 40->45, `city_watch` 30->35.
- Почему так: ветка была прокси для «формула может ошибочно срезать реальную городскую новость». Судья — это и есть замена прокси; держать оба означало иметь судью без предмета суждения.
- Ожидаемый эффект и метрика: `last_24h` защищено 82->16 (решает судья 83 из 99), `city_watch` 38->18 (56 из 74), `tech_business` 2->0 (19 из 19).
- Файлы/места: `llm_rewrite.py:_must_translate_before_cap`, `REWRITE_SHORTLIST_CAPS_BY_BLOCK`.
- ПРОВЕРКА: на реальном пуле 23.07 несудейские блоки не сдвинулись вовсе (`weekend_activities` 44, `next_7_days` 6 — как было); `reject` в `last_24h` стал исполняться `1` вместо `0`, оставшиеся `2` заблокированы настоящими защищёнными полосами; `unittest discover` 910/910.
- Удалено: безусловное освобождение от лимита по паре категория+tier внутри судейских блоков; для остальных блоков правило сохранено без изменений.

### 0153 — Follow-up с новыми фактами больше не выпадает по совпадению персоны — 2026-07-23
- Статус: внедрено; красный тест был предсуществующим и теперь зелёный.
- Проблема: `test_cross_day_same_victim_with_new_fact_passes` падал до этой ветки. Новость «офицеру предъявлено обвинение, суд 16.09.2026, иск £250 000» получала `dedupe_decision=drop` и `include=False`, хотя `visible_repeat_verdict.allow=True`, `repeat_class=lifecycle`, `has_new_story_phase=True`.
- Причина (корень): ветка, применяющая разрешающий вердикт, требовала `previous is not None` — то есть совпадения по точному fingerprint. Кросс-дневный follow-up приходит через people/topic-совпадение и наполняет `similar_previous`, оставляя `previous` пустым. Вердикт вычислялся и не применялся; кандидат доезжал до guard'а невалидного решения и отбрасывался вместе с реальными новыми фактами.
- Решение: разрешающий вердикт применяется при ЛЮБОЙ предыдущей ссылке — `previous` или `similar_previous`.
- Почему так: рядом стоит собственный комментарий модуля «единый repeat-вердикт, чтобы ни одна поздняя стадия его не переопределила». Ветка нарушала это правило по типу совпадения.
- Ожидаемый эффект и метрика: развитие сюжета с новой фазой (обвинение, приговор, дата суда) переживает совпадение по фигуранту; запрещающий вердикт по-прежнему ведёт к drop.
- Файлы/места: `dedupe.py:dedupe_candidates`.
- ПРОВЕРКА: `unittest discover` 910/910; до правки тот же тест падал и на чистом HEAD (проверено отдельным worktree), то есть поломка не была внесена этой веткой.
- Удалено: требование точного fingerprint-совпадения для применения разрешающего repeat-вердикта.

### 0154 — Стенд подбора промпта судьи на сохранённых новостях — 2026-07-23
- Статус: код готов и проверен вхолостую; прогон с моделью не выполнен — ключей в рабочем окружении нет.
- Проблема: промпт судьи выбран по рассуждению, а не по сравнению. Нужен способ прогнать варианты по ОДНОМУ и тому же реальному блоку.
- Решение: `tools/board_prompt_ab.py` — три варианта (`v1_baseline`, `v2_named_failure` с явным описанием наблюдавшегося провала, `v3_criteria` с разложением на consequence/actionability/novelty) по одному сохранённому пулу, плюс попарное согласие и проба на перекос.
- Почему так: проба на перекос не требует разметки — она меряет ровно известный дефект (средний ранг рутинных уведомлений против хардньюс). Вариант, ставящий дорожные работы выше суда, отбраковывается сразу, до ручной оценки.
- Ожидаемый эффект и метрика: выбор промпта по числам; полные порядки сохраняются в `data/validation/board_prompt_ab_<block>.json`.
- Файлы/места: `tools/board_prompt_ab.py`, `tools/board_eval.py`.
- ПРОВЕРКА: без ключей стенд выходит с понятным сообщением и не падает; три варианта промпта подтверждённо различны. Числа сравнения будут получены на окружении с ключами.
- Удалено: ничего.

### 0155 — Промпт судьи переписан под наблюдавшийся дефект; порядок подачи перемешивается — 2026-07-23
- Статус: внедрено; прогон с моделью не выполнен (ключей в рабочем окружении нет).
- Проблема: первый промпт описывал цель абстрактно («суди важность, а не полноту») и подавал кандидатов в порядке формульного балла. Первое не лечит конкретный дефект, второе прямо противоречит задаче: судья видел ранжирование той самой формулы, которую должен перепроверять.
- Причина (корень): промпт писался рассуждением о цели, а не от зафиксированного провала; порядок подачи вообще не рассматривался как вход.
- Решение: (1) конкретный анти-пример из production в начале промпта — остановка выше суда и объяснение, почему это ошибка; (2) три критерия с явным весом, actionability прямо назначен тай-брейкером, а не ведущим — именно его перевес и породил дефект; (3) запрет ранга за известность имени и за объём деталей; (4) `board_note` первым полем ответа — принудительный проход по всему списку до фиксации порядка; (5) reject сужен: проигрыш конкуренции = backup, не reject; (6) детерминированное перемешивание порядка подачи по ключу `дата|блок`.
- Почему так: LLM-ранкеры документированно смещаются позицией элемента в промпте и популярностью; подача в порядке формулы делала судью её эхом. Перемешивание по фиксированному ключу убирает подсказку и сохраняет воспроизводимость повторного прогона того же утра.
- Ожидаемый эффект и метрика: `board_rank_report.blocks[*].board_note` показывает, что судья считает сильнейшим сигналом дня; `top3` с обоснованием.
- Файлы/места: `board_rank.py:BOARD_RANK_SYSTEM/_call_block/_parse_board_rank_results/rank_boards`.
- ПРОВЕРКА: `unittest discover` 910/910; перемешивание детерминировано по `дата|блок`. Числа качества ранжирования будут получены на первом прогоне с ключами.
- Удалено: подача кандидатов в порядке `section_board_score`; абстрактная формулировка запрета без примера.

### 0156 — `planning_civic` перестала быть защитой от лимита в судейских блоках — 2026-07-23
- Статус: внедрено; измерено на реальном пуле 23.07.
- Проблема: после 0152 в `city_watch` оставалось 18 защищённых, из них 11 держались только на полосе `planning_civic` — «Young competition winner announced», «Historical Plantation Lodges open to all», «Building consultancy eyes growth after BGF backing», «Manchester builds more homes than Greater London».
- Причина (корень): `planning_civic` присваивается по `story_type in {planning, civic, local_cost}` или regex — это ярлык ТЕМЫ, а не признак «нельзя резать». В `protected_lane` он лежит рядом с `public_safety` и `transport`, которые действительно неотчуждаемы.
- Решение: внутри `JUDGED_BLOCKS` освобождение от лимита дают только полосы `public_safety` и `transport`. Несудейские блоки используют полосы как раньше, без изменений.
- Почему так: конкурс благоустройства и рост консалтинговой фирмы — это ровно те решения, ради которых судья и заводился. Убийство, обвинение и пожар — нет.
- Ожидаемый эффект и метрика: `last_24h` защищено 16->14 (решает судья 85 из 99), `city_watch` 18->7 (67 из 74).
- Файлы/места: `llm_rewrite.py:_must_translate_before_cap`.
- ПРОВЕРКА: реальный пул 23.07 — в `last_24h` остались только `public_safety` (убийства, обвинения, пожары, аресты, смерть экс-депутата); `weekend_activities` 44, `next_7_days` 6, `transport` 12 не изменились; `unittest discover` 910/910.
- Удалено: освобождение от лимита по полосе `planning_civic` внутри судейских блоков; фикстура `test_strong_fresh_news_survives_global_rewrite_board_cap` расширена на несколько блоков, потому что один блок больше не может переполнить общий лимит 90 в одиночку.

### 0157 — География судьи — контракт раздела, а не выпуска — 2026-07-27
- Проблема: судья снял две русскоязычные строки из Лондона как «не Greater Manchester», хотя `Русскоязычные концерты и стендап UK` — UK-блок по контракту. Liverpool/Daresbury из Future он при этом снял верно.
- Причина (корень): в промпте стояло одно общее правило географии на весь выпуск («не выдавать не-GM за GM»), у строки не было признака её собственной геозоны — судья применял GM ко всем разделам одинаково.
- Решение: у каждой строки в payload появился `geo_scope` (`gm` | `uk` | `uk_outside_gm`), промпт разложен по контракту раздела, а детерминированный замок `_drop_out_of_contract_geo_rows` выбрасывает geo-действие, если раздел эту географию разрешает. Отклонённое пишется в `geo_contract_rejected` отчёта.
- Почему так: одного текста промпта мало — модель уже ошибалась ровно здесь; замок делает разрешённую географию неснимаемой независимо от формулировки ответа.
- Ожидаемый эффект и метрика: Russian-строки из Лондона доезжают до читателя; Future/Ticket по-прежнему теряют не-GM.
- Файлы/места: `pre_send_quality_judge.py:SYSTEM_PROMPT/_SECTION_GEO_SCOPE/section_geo_scope/_drop_out_of_contract_geo_rows/_line_payload_for_judge/_apply_repair_executor`.
- ПРОВЕРКА: `unittest discover` 914 тестов, падают только два досуществовавших date-dependent (проверено на чистом worktree HEAD); новый тест строит HTML по разметке реального выпуска.
- Удалено: глобальное правило судьи «любое событие должно быть в GM»; общая геопроверка всех строк одним условием.

### 0158 — Ночная Food-карточка обязана получить решение дедупа — 2026-07-27
- Проблема: из 5 ночных Food-карточек ни одна не дошла до выпуска; Rudy's и Fosforo Lounge остались с `reason="pending dedupe"` при `dedupe_decision=drop` — то есть без объяснимого решения после завершения стадии.
- Причина (корень): «pending dedupe» — заглушка склада, но ветки дедупа пишут причину как `candidate.get("reason") or "…"`. Непустая заглушка считалась готовой причиной и переживала стадию.
- Решение: (1) заглушка снимается ДО решения, каждая ветка пишет свою настоящую причину; (2) `close_pending_dedupe_reasons` — замок на выходе стадии, пишется в `pending_dedupe_closed`; (3) полнота блока считается после дедупа (`post_dedupe_completeness`, только пережившие), пороги остались registry-шные: floor 3 строки, min_sources 2; в плане появились `planned_sources`/`min_sources`; (4) `prune_inventory` удаляет копию openings-записи, чья ссылка уже лежит в каноническом `weekend_activities`.
- Почему так: причина снятия — часть контракта карточки; без неё RCA упирается в заглушку. Полнота до дедупа считала карточки, которых в выпуске уже не будет.
- Ожидаемый эффект и метрика: на реальном складе 27.07 удалены 3 копии (Asian Food Night Market июль/июнь + Makers Market), food_openings 27→24.
- Файлы/места: `dedupe.py:_is_pending_dedupe_reason/close_pending_dedupe_reasons/dedupe_candidates`, `inventory.py:_superseded_block_copies/prune_inventory`, `plan_digest.py:sections_summary`.
- ПРОВЕРКА: `prune_inventory` прогнан на копии реального `data/state/inventory` — ровно три ссылки, обе Asian Food Night Market в их числе; `unittest discover` без новых падений.
- Удалено: возможность оставить `pending dedupe` после завершения дедупа; Food-копии карточек, канонически живущих в Weekend.

### 0159 — Today наполняется по смыслу сегодняшнего действия — 2026-07-27
- Проблема: блок «Что важно сегодня» пуст четвёртый день. Upstream отдавал две карточки, обе непригодные; план правильно отказывается добирать из Fresh/City.
- Причина (корень): отбор в Today шёл по списку ключевых слов и баллу (`police`, `council`, `fire`), а не по тому, есть ли у читателя действие на сегодня. Слово «council» в тексте — не повод для блока.
- Решение: `_today_focus_native_fit` — карточка попадает в Today, только если сегодняшнее действие относится к одному из пяти классов (ограничение, срок, изменение услуги, погодное воздействие, активная проблема безопасности) и в её собственном тексте есть место и затронутые люди. Балл остался только упорядочиванием уже пригодных. Решение принимается по редакционным полям: `evidence_text` тянул чужую обвязку страницы («M60 lanes shut» приезжало в карточку про суд).
- Почему так: класс действия — это и есть причина существования блока; ключевое слово ей не эквивалентно.
- Ожидаемый эффект и метрика: на реальном пуле 27.07 из 33 кандидатов шлюз пропускает одну реальную карточку (M60: полосы закрыты, водители в задержках) и отсекает ложную (бан одного человека в больнице).
- Файлы/места: `collector/routing.py:_TODAY_ACTION_CLASS_RE/_today_blob/_today_action_class/_today_focus_native_fit/_promote_to_today_focus`.
- ПРОВЕРКА: шлюз прогнан на реальном `candidates.json` 27.07; `unittest discover` без новых падений.
- Удалено: мёртвый recovery-контур писателя — `_TODAY_FOCUS_BOARD_SOURCE_SECTIONS`, `_TODAY_FOCUS_RECOVERY_SOURCE_BLOCKS`, `_today_focus_bucket`, `_today_focus_candidate_score`, `_today_focus_recovery_line` и два теста, проверявших только его. Активная `_today_focus_candidate_is_eligible` оставлена.

### 0160 — Next7 — недосуговый блок — 2026-07-27
- Проблема: 42 кандидата в Next7, прошло 0. В блок попадали концерты, рынки и обычные билеты, после чего с них требовали дату, район и цену.
- Причина (корень): три маршрута сливали досуг в Next7 (обычный будущий билет с датой ≤7 дней, Weekend-событие на 4–7 дней), а `event_quality` требовал цену/бронирование от всего, что лежит в событийном блоке — включая ограничения и сроки, где цены не бывает.
- Решение: обычный будущий билет уходит в `future_announcements` независимо от даты; Weekend-событие на 4–7 дней демотируется в `future_announcements`; проверка «цена/бесплатно/бронирование» не применяется к недосуговой карточке Next7 (`_is_non_leisure_next7`).
- Почему так: Next7 — практический блок ближайшей недели (ограничения, сроки, официальные изменения услуг); досуг уже имеет свои три дома — Weekend, Ticket, Future.
- Ожидаемый эффект и метрика: Next7 перестаёт быть свалкой афиши; недосуговая карточка больше не отбраковывается за отсутствие цены.
- Файлы/места: `collector/routing.py:_adjust_ticket_radar_block`, `candidate_validator.py:_demote_distant_weekend_event`, `event_quality.py:_is_non_leisure_next7/event_quality_report`.
- ПРОВЕРКА: `unittest discover` без новых падений; новый тест проверяет отсутствие `price_or_free_or_booking` в missing для недосуговой Next7-карточки.
- Удалено: перенос обычных будущих билетов в `next_7_days`; перенос Weekend-событий 4–7 дней в Next7; требование цены/бронирования для недосуговых Next7-карточек.

### 0161 — Слот плана только с доказанным путём до HTML — 2026-07-27
- Проблема: план назначил Tech 2, Russian 2, Future 7, в HTML осталось 0, 0 и 4; запасных этим разделам план не дал ни одного.
- Причина (корень): проверку «есть ли путь к строке» (`_backup_render_path`) проходили только запасные — основной слот выдавался без неё. Плюс цепочки запасных набивались по слотам подряд: первый слот забирал всю очередь, обязательный слот ниже оставался пустым.
- Решение: (1) основной слот допускается только с непустым `plan_render_path` (иначе `primary_ineligible:no_render_path`); (2) запасные раздаются в два круга — сначала по одному каждому слоту, начиная с обязательных, потом углубление до `depth`; (3) в разделе появились `required_slots_without_backup_count`, `planned_sources`, `min_sources`; предупреждение поднимается только когда запасные были, а обязательный слот всё равно остался без них (пустой резерв — это уже недобор пула).
- Почему так: цепочка слот→запасной уже существует и работает для всех разделов; чинилась не глубина, а порядок раздачи и вход в основной слот. Нового cross-block fallback не добавлено.
- Ожидаемый эффект и метрика: план на пуле 27.07 — 97 слотов, writer исполняет их без ошибок; карточка совсем без фактов теперь снимается планёркой, а не писателем.
- Файлы/места: `plan_digest.py:run_plan_digest` (вход в пул, раздача запасных, sections_summary).
- ПРОВЕРКА: plan-digest + write-digest прогнаны на копии реального `data/state` 27.07 (93–95 строк, `errors: []`); `unittest discover` 914 тестов; фикстура `test_dedupe_and_show` обновлена — карточка без фактов больше не доезжает до писателя.
- Удалено: ничего.

### 0162 — Transport: закончившееся окно и смысловой дубль участка — 2026-07-27
- Проблема: в выпуске после 08:31 стояла авария «ожидается до 08:00»; рядом официальный статус Eccles line и медийная статья BBC описывали одни и те же работы.
- Причина (корень): у транспортной карточки проверялась только дата (retention/prune по дню), время суток окончания не разбиралось вовсе. Смысловой дубль не ловился: `_apply_intra_batch_dedup` намеренно пропускает пары transport-transport, а собственной проверки «официальный статус против пересказа» не было.
- Решение: `transport_end_datetime`/`transport_card_is_finished` разбирают «until 08:00» / «до 08:00 27 июля 2026»; `_expire_finished_transport` снимает карточку, чьё окно закончилось до времени планирования, `passes_morning_contract` не выдаёт такую запись в утренний отбор (`transport_window_ended`); `_collapse_transport_segment_duplicates` оставляет по участку официальный пассажирский статус, а медийную статью допускает только с классом последствий, которого нет ни в одном официальном статусе того же участка.
- Почему так: ещё одно название места внутри тех же работ — не самостоятельное воздействие; самостоятельным считается новый класс последствий (деньги, забастовка, отдельное закрытие).
- Ожидаемый эффект и метрика: на реальном состоянии 27.07 транспорт сокращается с 5 строк до 3 — уходят «до 08:00» и пересказ BBC, остаются три официальных статуса.
- Файлы/места: `transport_card.py:transport_end_datetime/transport_card_is_finished`, `transport_fill.py:_expire_finished_transport/_collapse_transport_segment_duplicates/run_transport_fill`, `inventory.py:passes_morning_contract`.
- ПРОВЕРКА: transport-fill + plan + write прогнаны на копии реального `data/state` 27.07, транспортная секция выведена глазами (3 строки); `unittest discover` без новых падений.
- Удалено: завершившиеся transport-карточки из serving inventory и утреннего отбора; более слабая строка смыслового дубля участка.

### 0163 — Пустой план не останавливает выпуск — 2026-07-27
- Проблема: `write_digest` возвращал `ok=False` («No release plan»), если в плане ноль слотов. Это единственная точка, где редакционная пустота, а не техника, тормозила весь выпуск — против контракта «обогатить → заменить → пропустить, блокирует только техника».
- Причина (корень): одно условие смешивало два разных отказа — отсутствующий/нечитаемый run-specific state (техника) и план без слотов (редакционный недобор).
- Решение: условия разведены. Нет самого файла плана → по-прежнему технический стоп. План есть, но состав пуст → писатель пишет честно пустой черновик, ставит `stage_status="complete_degraded"` + `empty_plan=true` и возвращает `ok=True`; решение принимает release-гейт. Ленивая планёрка перед этим по-прежнему пытается собрать состав. `_validate_stage_reports` принимает `complete_degraded` как рабочий статус стадии.
- Почему так: владельцем решения «отправлять или нет» остаётся release-гейт; писатель обязан отдать результат, а не остановить конвейер.
- Ожидаемый эффект и метрика: на копии реального состояния 27.07 с обнулёнными кандидатами — writer `complete_degraded`, `errors: []`, build `ok=True`, `release_decision=ship_degraded`, черновик промотирован в outgoing. На нормальном состоянии поведение не изменилось (95 строк, `complete`).
...
### 0170 — Нативная Today-карточка проходит тот же шлюз — 2026-07-27
- Проблема: шлюз 0159 применялся только к кандидатам, повышаемым из `last_24h`/`city_watch`. Источник может отдать карточку прямо в блок (`data/sources.toml`: GMMH, `primary_block = "today_focus"`) — такая карточка обходила `_today_focus_native_fit`, а `_today_focus_substantive` считала её полноценной без проверки действия, места и затронутых людей. Блок выглядел набранным на 2 строки, в выпуск из него уходило 0.
- Причина (корень): проверка стояла на маршруте повышения, а не на входе в блок; счётчик полноты считал по `primary_block`, не спрашивая, прошла ли карточка шлюз, и не глядя на `include`.
- Решение: `_demote_unfit_native_today` прогоняет через шлюз ВСЕ карточки блока (включая помеченные как повышенные — флаг мог остаться в state от прошлого прогона) и уводит непригодные в `city_watch` с причиной ДО подсчёта полноты; из выпуска карточка не теряется. `_today_focus_substantive` дополнительно считает только публикуемые (`include`) строки.
- Почему так: блок наполняется по одному правилу независимо от того, кто предложил карточку; иначе полнота считается по строкам, которые сам блок не пропустил бы.
- Ожидаемый эффект и метрика: на реальном пуле 27.07 обе нативные Today-карточки («Trafford WW2 bunker», «Rochdale man charged») уходят в City с `no_today_action`, полнота 2 → 0, и полный проход промоции ставит в блок одну реальную карточку (M60, restriction, include=True).
- Файлы/места: `collector/routing.py:_demote_unfit_native_today/_today_focus_substantive/_promote_to_today_focus`.
- ПРОВЕРКА: прогон `_promote_to_today_focus` на реальном `candidates.json` 27.07 (демоция обеих нативных, итог — 1 пригодная карточка); `unittest discover` 922 теста, падают только два досуществовавших date-dependent.
- Удалено: подсчёт полноты Today по карточкам, не прошедшим шлюз блока, и по непубликуемым строкам.

### 0170 — 0164 и 0167 не переживали склад; чинится снимком фактов и подтверждённым жанром — 2026-07-27
- Проблема: обе доработки работали только внутри одного прогона. (1) У всех 14 карточек с `llm_cv_matched` после `inventory_record_to_candidate` вердикт признавался неактуальным, статус утром снова сбрасывался в `needs_llm_cv_match`, то есть модель звали второй раз. (2) Все складские карточки восстанавливались с `include=True`, и 4 ночных вердикта `skip` возвращались в утренний пул. (3) Подтверждённый жанр Pop/Pop после round-trip становился пустым.
- Причина (корень): (1) хеш считался из `_llm_payload`, где `price_or_access` берётся из `free_access_reason` — а его перезаписывает сам вердикт (`match.update`) сразу после записи хеша, поэтому снимок не совпадал сам с собой; вдобавок `price` не хранился в `fact_card` и терялся. (2) `inventory_record_to_candidate` выдавал фиксированный `include=True`: решение ночи вообще не сохранялось. (3) склад хранил только `subgenre`, а рендер требует совпадения `genre == subGenre` — грубая категория не восстанавливалась, и проверка совпадения проваливалась всегда.
- Решение: (1) `professional_cv_fact_snapshot` — неизменяемый снимок только исходных фактов (id, название, дата, место, price/free источника, ссылка, источник, описание); ничего производного от вердикта в него не входит; снимок и его хеш кладутся в `professional_llm_match`; `price`/`free` добавлены в `fact_card` и восстанавливаются. (2) `professional_cv_outcome` и `professional_cv_visible` сохраняются в `fact_card`, `_restore_night_cv_decision` возвращает решение ночи: skip — снят, held — удержан, go/consider — виден. (3) вместо `subgenre` склад пишет `confirmed_subgenre`, вычисленный в момент сбора, пока оба поля источника ещё на руках; рендер берёт его напрямую, а противоречивая грубая категория не хранится вовсе.
- Почему так: снимок обязан быть неизменяемым — любое поле, которое пишет сам вердикт, делает сравнение невозможным. Жанр подтверждается там, где есть оба поля источника, а не там, где рисуется строка.
- Ожидаемый эффект и метрика: на реальном складе 27.07 — 14 из 14 вердиктов остаются актуальными после round-trip и утреннего пересчёта (было 0 из 14); 3 go и 7 consider видимы, 4 skip не возвращаются в пул. Жанр: 22 Pop, 22 R&B, 16 Country, 10 Jazz, 8 Reggae переживают склад без изменений, выдуманных жанров не появляется.
- Файлы/места: `professional_events.py:professional_cv_fact_snapshot/professional_cv_evidence_hash`, `inventory.py:build_inventory_record/inventory_record_to_candidate/_restore_night_cv_decision/_confirmed_subgenre`, `writer.py:confirmed_ticket_subgenre/_ticket_genre`.
- ПРОВЕРКА: оба сценария прогнаны на реальном `data/state/inventory` через `build_inventory_record` → `inventory_record_to_candidate` → утренний `apply_professional_event_match`; тесты 0164 и 0167 переписаны так, что идут через склад (прежние проходили мимо него и потому не поймали баг); `unittest discover` 922 теста, падают только два досуществовавших date-dependent.
- Удалено: зависимость снимка CV-фактов от полей, которые перезаписывает вердикт; безусловный `include=True` при восстановлении профессиональной карточки; хранение неподтверждённого `subgenre`.

### 0171 — P0: единый контракт блоков, календарный A-tier и замена до снятия — 2026-07-27
- Проблема: planner/inventory/rewrite/judge использовали разные минимумы, географию и правила повторов; rewrite удерживал две отклонённые IT-карточки своим floor=3; A-tier возвращался в план ежедневно вопреки дедупу; completeness принимал `death sentence` и имя Thy Art Is Murder за уголовные факты; `strip` судьи снимал слот без запасного; Today считался заполненным до validator/curator; Next7 не имел производителя недосуговых D+2…D+7 изменений.
- Причина (корень): продуктовый контракт был размножен по четырём стадиям; A-tier override стоял до календаря; reverse fact-lock читал сырой заголовок, а не структурированный story frame; цепочка запасных вызывалась только для `patch/replace`; наполнение Today выполнялось в collector до отбраковки; маршрутизация Next7 умела выгонять досуг, но не умела принимать будущие практические изменения.
- Решение: (1) `block_policy.py` стал единственным 17-блочным реестром с `min/max/geo_scope/schedule/required_fields/repeat_policy/backup_depth`, версия `2026-07-27.p0`; common/inventory/planner/rewrite/judge только выводят из него свои представления, все их отчёты несут `block_policy_version`. (2) A-tier остаётся без количественного cap, но повтор разрешён только на первом обнаружении, изменении факта/статуса, старте продаж или D−365/30/7/3/1/0; planner не воскрешает upstream calendar-reject. Conservation считается по `artist+venue+date`, несколько URL и tour-card учитываются как представление физических событий. (3) completeness активируется только по структурированному story frame; `death sentence` нейтрализуется как известная метафора, а русские `убил/убийство` сохраняют death-concept. География judge берётся из registry, Russian остаётся UK. (4) любое снятие judge сначала пробует цепочку слота; при пустом резерве пишет `no_eligible_backup`; writer может потратить только 6 из 8 общих попыток до judge. (5) collector больше не наполняет Today; тот же смысловой шлюз запускается после validator и повторно после curator. (6) Next7 получает только датированные практические изменения D+2…D+7; досуг остаётся в Weekend/Ticket/Future.
- Почему так: это закрывает класс рассинхронизации одним источником решения и использует существующие plan/plan_execution/repeat/story-frame контуры; новых стадий, моделей и очередей не добавлено.
- Ожидаемый эффект и метрика: IT board-reject не защищается чужим минимумом; Russian London не снимается; пять ложных удалений 27.07 не повторяются; A-tier не повторяется ежедневно и не ограничивается cap; каждый доступный запасной распределяется первым кругом по слотам и используется до снятия; Today/Next7 показывают только реально подходящие карточки или честный shortfall.
...
### 0183 — Verify — единственная финальная истина состава — 2026-07-27
- Статус: внедрено; production-proof ожидается на следующем утреннем выпуске.
- Проблема: release считал event completeness по старому `draft_line`, underflow — до девяти возможных снятий судьёй, а source funnel приписывал planner-loss писателю/редактору; judge и verify публиковали разные числа секций.
- Причина (корень): финальный результат независимо реконструировали release, judge и verify, хотя только verify уже сопоставляет каждый slot/fingerprint с фактической строкой отправляемого HTML.
- Решение: `verify_digest_plan_report` теперь один хранит финальные `shortfalls`, event completeness по реально потреблённой HTML-строке и source funnel `raw → curated → ranked → planned → written → final`. Rank сохраняет неизменяемый входной `curated_for_rank`; planner учитывается по primary и backup fingerprints. Release удаляет старый funnel/event/underflow и маркирует свою source/composition диагностику как `intermediate_pre_verify`; judge называет свои counts/alerts входными до собственных правок. Старый параллельный event-completeness predicate удалён.
- Почему так: защита plan/repair/verify не ослаблена; устранены только три конкурирующих определения результата. Потеря на planner теперь видна как `loss_stage=planned`, а снятие после writer — как `loss_stage=final`.
- Ожидаемый эффект и метрика проверки: итоговый отчёт показывает фактические даты/площадки по slot/fingerprint, отсутствующие Today/Next7/Professional/Russian — в одном `shortfalls`, а каждый источник имеет шесть отдельных колонок без ложного «lost at writer/editor».
- Файлы/места: `verify_digest_plan.py:_final_event_completeness/_final_source_funnel/run_verify_digest_plan`; `llm_rewrite.py:run_rank_digest`; `release.py:build_release`; `pre_send_quality_judge.py:_product_completeness_context`; `tests/test_plan_contract.py`; `tests/test_inventory.py`.
- ПРОВЕРКА (offline): сквозной regression подтвердил `Planner Lost = 1 raw / 1 curated / 1 ranked / 0 planned / 0 written / 0 final`, `loss_stage=planned`; финальная event-card дала `checked=1, missing_date=0, missing_venue=0`; Next7 shortfall берётся из verify. Food TTL fixture заморожен на 20.07 и снова зелёный. Семь целевых контрактов OK. Replay 27.07: plan 48 slots, 45 shown, 4 removed, lead `ok`, `blank_runs_2plus=0`, technical verify OK, `ship_degraded` только по девяти содержательным divergences.
- Где была ошибка (если не сработает): проверить `final_selection_report.final_rows[*].final_html_line/final_candidate`, затем `verify_digest_plan_report.source_funnel`; release больше не является источником финального состава.

### 0184 — Metrolink: полное ограничение хранится до даты окончания — 2026-07-28
- Статус: внедрено локально; production-proof ожидается на следующем утреннем выпуске.
- Проблема: в выпуске 28.07 осталась только Eccles line, хотя сохранённые факты по работам 13.07–02.08 называют шесть затронутых линий и три разных режима движения.
- Причина (корень): `active_tram_disruptions.json` сохранял одну извлечённую `line`, а парсер отдельной даты ошибочно превратил уже прошедшее начало в `2027-07-13`; синтетический reminder поэтому не содержал полный сетевой режим.
- Решение: активная запись хранит структурированные `affected_services`; неполная legacy-запись один раз восстанавливается из уже сохранённого `published_facts.json` без live fetch. Reminder группирует все ограничения в одной строке, появляется только при `start_date <= today <= end_date` и удаляется после конца. Диапазон дат нормализуется как единое окно, поэтому начало не переносится на следующий год.
- Почему так: повторный ежедневный сбор не нужен и не является условием показа; сеть может только дать обновлённые факты. Отдельные карточки на каждую линию не создаются, чтобы planner/dedupe не схлопывал один инцидент и транспортный блок не раздувался повторами.
- Ожидаемый эффект и метрика проверки: до 02.08 включительно один reminder называет Altrincham, Eccles, Trafford Centre, Manchester Airport, East Didsbury и Rochdale; после 02.08 запись автоматически pruned.
...
- Статус: внедрено; production Events-proof работает, утренний Today/Next7 proof ожидается в штатном выпуске.
...
- ПРОВЕРКА: до изменения реальный state 28.07: Weekend `404 records / 50 fact_ready / 41 render_ready / 0 eligible` во вторник — накопление подтверждено без преждевременного расхода; после нового predicate Today имеет ровно две реальные карточки (Stockport bins и GM motorway closures), а исторически закрытый Town Hall не проходит. Registry probe: `static_next_7=[]`; contract tests и полный набор 955 тестов OK. Production Events-wave `30343992976` на commit `c35c00c`: 55/55 sources fetched, 0 source errors, 141 found / 141 enriched / 37 fact-ready; записано 129 новых culture records, все `129/129 → weekend_activities`, `0 → next_7_days`, из них 32 `ready/render_ready`. На вторник `morning_eligible=0`, то есть склад пополнился, но досуг не был преждевременно выдан.
...
### 0191 — Today: словарь мест, метафора «flooding» и аудитория по объекту; Next7: дата прозой — 2026-07-28
- Статус: внедрено; замерено на реальном пуле 28.07. Дополняет 0189 — там шлюз Today и реестр Next7, здесь три оставшихся отсекателя.
- Проблема: после 0189 из 38 кандидатов Today проходили 2, а динамический роутер Next7 не срабатывал ни разу (0 из 7 практических карточек). Три причины: (1) `GM_TOKENS` не знал Cheetham Hill — карточка про оцепление после поножовщины снималась как `no_place`; (2) класс `weather_impact` ловил метафору «a man involved in flooding GM with GBL» — полицейский рейд получал погодный класс; (3) `_next_7_structured_date` принимал только ISO/структурную дату, а практическое изменение почти всегда названо прозой.
- Причина (корень): словарь мест собирался по центральным районам и не содержал 17 реальных районов и городов GM, встречающихся в пуле 40+ раз; «flooding» стоял в регулярке без погодного контекста; поставка Next7 требовала формата данных, которого у её источников нет.
- Решение: (1) в `GM_TOKENS` добавлены 10 однозначных имён (Cheetham Hill, Blackley, Timperley, Failsworth, Milnrow, Mossley, Horwich, Westhoughton, Walkden, Urmston); (2) погодные термины требуют контекста (`flood warning/alert/risk`, `flooded …`, `flooding of/expected/forecast`); (3) `_next_7_structured_date` при отсутствии структурной даты берёт ближайшую будущую дату из СОБСТВЕННЫХ полей карточки через существующий `_explicit_dates_from_blob` — окно D+2…D+7 по-прежнему проверяет вызывающий; (4) аудитория считается названной, если карточка называет затронутый общественный объект или маршрут (street/road/line/school/hospital/library/park/…), но только для классов restriction/active_safety/service_change и только пока действие длится.
- Почему так (отвергнутая альтернатива): владелец одобрил правило «объект вместо людей» без оговорок, но замер показал, что в чистом виде оно впускает две негодные карточки — реставрацию ратуши («closed since 2018») и суд по прошлому bomb hoax, — и ни одной годной. Поэтому правило внедрено с фильтром завершённого действия (`since 20xx`, `months away`, `charged/convicted/in court`), и оба ложных случая остаются снятыми. Прозаическая дата берётся только из своих полей: полный `evidence_text` вернул бы чужие даты из обвязки страницы.
- Ожидаемый эффект и метрика: Today на пуле 28.07 — 2 → 3 карточки (Stockport bins, оцепление в Cheetham Hill, motorway closures), ложных нет; полицейский рейд больше не weather_impact; Next7 — 0 → 1 маршрутизированная карточка (by-election 30.07, D+2).
- Файлы/места: `collector/filters.py:GM_TOKENS`, `collector/routing.py:_TODAY_ACTION_CLASS_RE/_TODAY_AFFECTED_PLACE_RE/_TODAY_FINISHED_ACTION_RE/_today_focus_native_fit/_next_7_structured_date`.
- ПРОВЕРКА: прогон шлюза и роутера на реальном `data/state/candidates.json` 28.07 (числа выше, каждая карточка просмотрена глазами); `unittest discover` — 955 тестов OK, добавлен 1 тест на пару «оцепление проходит / ратуша не проходит».
- Удалено: срабатывание погодного класса по слову «flooding» без погодного контекста.
- Не сделано (осознанно): «motorway closures from Monday to Thursday» в Next7 не попадает — нужен разбор дат по названию дня недели с якорем на дату публикации. Это новая способность, не правка; предлагается отдельным пунктом.

### 0192 — YouTube заменяет платную Spotify-зависимость в рейтинге артистов — 2026-07-28
- Статус: ПРОВЕРЕНО-работает.
- Проблема: Spotify Development Mode с февраля 2026 требует активную Premium-подписку владельца приложения. В последнем rank-run Spotify имел `no_credentials` для 158 из 158 фактически вызванных артистов; cache содержал 1883 артиста, только 1 A-tier и 138 незавершённых A-tier recheck. Платная зависимость не давала ни одного положительного сигнала и заставляла повторять неуспешные проверки ежедневно.
- Причина (корень): `_artist_notability` обязательно вызывал Spotify Search и считал `spotify_popularity/spotify_followers` единственным современным вторым сигналом для немануального A-tier; workflow передавал только Spotify credentials. Существующий 30-дневный `ticket_notability_cache.json` уже решал повторные запросы, но не хранил YouTube identity/statistics и суточную поисковую квоту.
- Решение: Spotify client/token/search удалены. Wikidata теперь читает официальный YouTube channel ID (`P2397`); при наличии ID используется дешёвый `channels.list`, иначе `search.list(type=channel)` с проверкой имени, music topic и запретом fan/tribute/unofficial. Cache v3 хранит channel ID/name, subscriber/view counts, hidden-subscriber flag, identity source/confidence и persisted `youtube_search_quota`; свежий результат используется 30 дней. Поиск ограничен 95 вызовами в день с сохранением счётчика между workflow reruns; очередь по-прежнему обслуживает ближайшие события первой. A-tier требует `YouTube subscribers >= 1m` плюс `Last.fm >= 250k` или `Wikidata >= 16 sitelinks`; YouTube views сохраняются для наблюдения, но не влияют на tier. Скрытые подписчики не превращаются в ложный ноль-сигнал. Workflow использует только `YOUTUBE_API_KEY`; Spotify secrets удалены. Для безопасного production-proof добавлен ручной input `stop_after_rank`: он печатает компактный provider-report и гарантированно пропускает plan/build/send/warnings/state commit, поэтому проверка ключа не создаёт второй Telegram-выпуск и не меняет production state.
- Почему так (отвергнутые альтернативы): отдельная CSV/Excel-таблица дублировала бы уже persisted JSON source of truth; YouTube search для каждого артиста превысил бы текущий дневной лимит 100 при наблюдавшихся 159 recheck; просмотры канала как основной порог и YouTube-only A нестабильны из-за Shorts, нескольких/фанатских каналов и несопоставимой шкалы. Last.fm-only по-прежнему даёт максимум B.
- Ожидаемый эффект и метрика: `rank_digest_report.ticket_notability.prefetched.provider_status.youtube.ok > 0`; `no_credentials=0`; `youtube_search_quota.search_calls <= 95`; повторный запуск в пределах 30 дней увеличивает `skipped_fresh` без повторного YouTube lookup; A-tier имеет `youtube_plus_independent_signal` либо исключительный Wikidata-сигнал.
...
### 0208 — Удаление старого reserve/quarantine/repeat-каркаса после Stage 3/4 — 2026-07-28
- Статус: внедрено; production-proof ожидается.
- Проблема: после переноса состава в `plan-digest` конфликтующие действия были отключены, но старые API и поля продолжали жить в коде: editor просил `block_actions` и игнорировал их, rank писал три прежних reserve-флага, release публиковал пустые quarantine/backup-поля и при отсутствии governing repeat сам повторно считал policy. Тесты закрепляли часть уже не вызываемых функций.
- Причина (корень): миграция сохраняла старую форму отчётов и private helpers «на совместимость», хотя производственных потребителей у них уже не было. Это скрывало незавершённое удаление и оставляло возможность вернуть второй источник решений.
- Решение: удалены editor reserve/block-action интерфейс и нулевые replacement-счётчики; writer reconcile/drop-bucket; `backup_pool_record` и TTL; `backup_pool_only/public_reserve/recoverable_reserve`; release quarantine/backup placeholders и неиспользуемый final-loss scan; мёртвые секции support-report. Release теперь только читает `governing_repeat_decision`: для ранее опубликованной строки без него пишет `missing_governing_repeat_decision`, но не запускает repeat-policy повторно. `REQUIRED/LOW_SIGNAL/SECTION/PUBLIC_SECTION/REWRITE_SHORTLIST` проекции удалены; planner, editor, release и CLI читают `block_policy` напрямую. Действующие `release_plan.slots[].backup_fingerprints` и judge slot replacement сохранены.
- Ожидаемый эффект и метрика проверки: в production-коде нет старых reserve/quarantine API и независимых таблиц правил; отсутствие planner repeat-решения обнаруживается как нарушение контракта; состав и настоящие slot backup chains не меняются.
- ПРОВЕРКА (offline): `py_compile` и `git diff --check` OK; 153 точечных planner/editor/inventory/rewrite теста, 342 writer/release/judge теста и полный набор 991 тест — OK. Replay 28.07 не изменился относительно кода до уборки: `39` слотов, `38 shown / 0 replaced / 1 removed`, `6` divergences, lead `ok`, `blank_runs_2plus=0`. Обязательный `replay_day.py --golden` завершил 12/12 дней без technical verify failure; на всех lead `ok`, новых пустых серий нет, метрики slot-plan/shown/replaced/removed совпали с baseline #0193–#0205.

### 0209 — Next7 понимает дни недели; Food и Professional повторяются только на календарных рубежах — 2026-07-28
- Статус: внедрено; offline/replay-проверка завершена, production-proof ожидается в следующем штатном выпуске.
- Проблема: `Что важно в ближайшие 7 дней` не принимало практические изменения с датой вида `from Thursday`; датированное открытие после первого анонса не могло вернуться перед открытием; Professional D1-карточка с подтверждёнными датой/местом и CV `consider` снималась старым `why_now_stale`, хотя planner уже разрешил календарный повтор.
- Причина (корень): Next7 читал ISO и день+месяц, но не относительный weekday; Food был жёстко исключён из общего calendar-carry; repeat-policy доверял сохранённому `editorial_contract`, который мог быть прикреплён до event enrichment; `_backup_eligible` позволял provisional reject пересилить финальное governing repeat-решение planner.
- Решение: weekday привязывается к дню публикации и используется только как будущая дата D+2…D+7; диапазон `from Monday to Thursday` якорится началом, поэтому уже начавшееся ограничение не маскируется под будущий четверг. Food получает собственные рубежи `D7/D1/D0` только при конкретной достоверной дате, названии и месте; недатированный анонс остаётся одноразовым. Для calendar-блоков контракт пересобирается из текущих event facts, обе стороны сравнения occurrence свежие. Разрешённый planner calendar/lifecycle verdict нейтрализует только старый `why_now_stale`, но не реальные validation errors.
- Почему это решает класс проблем: правило работает для любых практических weekday-изменений, любых датированных открытий и любых Professional events, а не для сегодняшних названий/URL; заполнение секций не достигается ослаблением факт-контракта или ежедневными повторами.
- Риски/компромиссы: Food без конкретной даты по-прежнему честно не повторяется; Next7 не интерпретирует голое название дня без связки `next/this/on/from/start`, чтобы соседний текст страницы не создавал ложную дату.
- Файлы/места: `collector/routing.py:_next_7_relative_weekday_date`; `block_policy.py:openings`; `editorial_contracts.py:calendar_repeat_review`; `repeat_policy.py:_contract/is_calendar_carry_candidate/visible_repeat_verdict`; `plan_digest.py:_backup_eligible`.
- ПРОВЕРКА: 5 точных регрессий и 234 затронутых routing/professional/repeat/planner теста OK; полный набор `995` тестов OK. Replay 28.07 до изменения: `39 public slots`, `38 shown / 0 replaced / 1 removed`, `6 divergences`, `11` секций, `37` bullets, lead `ok`. После: `40 public slots`, `39 shown / 0 replaced / 1 removed`, `5 divergences`, `12` секций, `38` bullets, lead `ok`; Professional восстановлен, технических нарушений плана и пустых серий нет. Food на этом историческом пуле остаётся пустым, потому что в нём нет будущей структурированной даты — правило не изобретает факт.

### 0210 — Pre-send editor: батч адресуется локальными индексами; правка не меняет ссылку строки — 2026-07-31
- Статус: внедрено; production-proof ожидается в следующем штатном выпуске.
- Проблема: утренний прогон `30611404460` (31.07) не отправил дайджест вообще: `Verify digest vs plan` вернул 4 технических расхождения (`planned_line_missing_from_final_html (lead)`, `removed_line_still_visible (city_watch-09)`, `line_outside_plan`, `final_report_row_count_mismatch`), шаг `Send digest to Telegram` — `skipped`. Изменений в коде с 28.07 не было.
- Причина (корень): `editor.py:_call_pre_send_russian_editor_batch` отправлял модели глобальные номера строк и принимал возвращённый `index` без проверки принадлежности батчу. В раунде 2 уходит отфильтрованный подсписок (`missing_action_indices: [40,42,43,44,45,48,51]`), модель ответила `0..6` — перенумеровала. `fixes[0]` применился к глобальной строке 0, то есть к лиду: `editor_report.pre_send_russian_editor` round 2, index 0, section «Главная история дня», `before: <b>…убийстве Уильяма Ридделла…</b> (bbc cqx72qw0r73o)` → `after: • …атаки собаки в Болтоне (bbc cvgx899j2d2o)`. Второй слой защиты тоже отсутствовал: `_line_preserves_links` (`editor.py:368`) сравнивает только количество ссылок, поэтому подмена URL прошла. Дальше каскад: лид продублировал `city_watch-09` → judge снял копию из радара (`unresolved`) → verify увидел расхождение плана и по `blocking_plan_kinds` (`verify_digest_plan.py:325`) заблокировал отправку.
- Решение: батч теперь адресуется локальными индексами `0..n-1`, ответ маппится обратно в глобальные; индекс вне батча отбрасывается (строка считается непокрытой, идёт в recovery, а не в чужой слот). На применении правки добавлена проверка идентичности ссылки: редактор правит слова, состав правит только план.
- Почему так (отвергнутые альтернативы): не менял `_line_preserves_links` — его же использует judge repair executor (`pre_send_quality_judge.py:1599`), где reserve replacement законно приносит другой URL. Не трогал текст промпта (иначе ломается hash в `prompts_meta`), правка чисто в транспортном слое.
- Ожидаемый эффект и метрика проверки: перенумерованный ответ модели больше не переписывает чужую строку; `verify_digest_plan` не получает `planned_line_missing_from_final_html (lead)` из-за редактора.
- Файлы/места: `src/news_digest/pipeline/editor.py:727` (локальные индексы + guard), `editor.py:884` (link identity), `tests/test_editor_line_index_mapping.py`.
- ПРОВЕРКА: новый regression воспроизводит production-баг на коде до фикса (`PYTHONPATH=<HEAD>/src` → `fixes = {0: '…Болтоне…'}`, `40 not found`) и проходит после. Полный набор командой CI `PYTHONPATH=src python3 -m unittest discover -s tests` — 996 тестов OK. Выпуск 31.07 не переотправлялся автоматически.

### 0211 — Same-run URL-дубль не воскресает повтором и не блокирует выпуск — 2026-08-10
- Статус: внедрено в `main` (`a0dfd1c`); production-proof новой логики ожидается на следующем плановом запуске. Выпуск 10.08 стартовал со старого `95a2cf0`, успешно отправлен и не является proof нового кода.
- Проблема: выпуск 09.08 (`Actions 31300170899`) был полностью собран, но не отправлен: verify вернул non-zero из-за `html_line_duplicated` для `pedddle.com/market/prestwich-makers-market`. Аудит показал второй такой же planned-дубль — Northern Quarter Makers Market; его pre-send LLM случайно заметил и заменил запасным, Prestwich не заметил. В collector `cheap_dedup_before_enrich` корректно снял 228 exact-дублей, но dedupe позже повторно включил 6 из них; все 6 дошли до `selected`.
- Причина (корень): cross-day `visible_repeat_verdict` в `dedupe_candidates` отвечал на другой вопрос («можно ли вернуть событие в новый день») и перезаписывал same-run решение `cheap_dedup_drop` («какая из двух строк одной страницы остаётся сегодня»). Planner дедуплицировал weekend по `(venue, date)` и весь выпуск по story key; две версии одного рынка имели разные venue/story keys, а canonical URL в составе и backup-цепочках не резервировалась. Writer учитывал только использованные fingerprint, поэтому обе пригодные строки успешно отрендерились и лестница `обогатить → запасной → снятие` не запускалась. Verify поздно увидел URL-дубль и ошибочно классифицировал исправимый контентный конфликт как техническую остановку.
- Решение: `story_intelligence` сохраняет `cheap_dedup_of`; dedupe восстанавливает exact same-run invariant до LLM/semantic review и повторно перед выходом. Planner безусловно исключает `cheap_dedup_drop`, резервирует каждый `canonical_url_identity` один раз среди lead, основных слотов и всех backup-цепочек и пишет число collision-demotions. Writer держит множество уже показанных canonical URL: поздний конфликт получает `duplicate_after_plan`, затем пробует следующего запасного слота, а при исчерпании цепочки снимается с этим кодом. Verify оставляет `html_line_duplicated` в divergences/`ship_degraded`, но больше не удерживает весь выпуск.
- Почему так (отвергнутые альтернативы): дополнительный prompt судье не решает класс — 09.08 одна пара была замечена, другая нет. Обогащение не помогает, потому что обе отдельные карточки уже факт-полны; дефект существует между слотами. Простое ослабление verify без раннего collapse оставляло бы читателю дубли, поэтому fail-soft классификация добавлена только вместе с детерминированными dedupe/planner/writer защитами.
- Ожидаемый эффект и метрика проверки: `cheap_dedup_drop && include=true` после dedupe = 0; canonical URL встречается в release plan не более одного раза среди primaries/backups; writer пишет `duplicate_after_plan` и использует backup; `verify_digest_plan.ok_technical=true`, отправка не блокируется URL-дублем.
...
### 0218 — Production inventory-first включён в `on` с автоматическим live-fallback — 2026-08-10
- Статус: ПРОВЕРЕНО-работает для production collect→rank; первый полный delivery в этом режиме — штатный выпуск 11.08.
- Проблема: после двух недель ночных волн production оставался в `assist`. Ночь реально обогащала/вставляла карточки, но утро всегда повторяло broad scan; при этом простая переменная `MORNING_INVENTORY_MODE=on` тоже не дала бы реального перехода, потому что после объединения block-policy все восемь стабильных блоков имели `source_replacement_allowed=false`.
- Причина (корень): capability `on` и строгий replacement-plan уже существовали, но два независимых rollout-выключателя остались закрыты: default/env был `assist`, а registry запрещал замещение каждой assist-категории. Поэтому код умел проверить current-night health, block sufficiency, source diversity и action-URL liveness, но результат всегда заканчивался `source_replacement_not_enabled`.
- Решение: production workflow явно ставит `MORNING_INVENTORY_MODE=on`, системный default также `on`; все восемь блоков с уже существующим режимом `assist` разрешают source replacement. Сам predicate не ослаблен: категория пропускает morning broad scan только если все её output-блоки assist, текущая ночная category-wave `ok`, обязательные блоки набрали floor/min_sources и достаточный набор action URL подтверждён `alive`. Иначе в том же run выполняется обычный live collection. Workflow печатает `mode`, фактический night→morning funnel, `night_replaced_categories` и причины каждого live fallback.
- Почему так (отвергнутые альтернативы): не переводим `lead/today/last_24h/city_watch/transport/football` из live/hybrid в складской publish — им нужна утренняя свежесть. Не считаем HEAD 403/unknown живой ссылкой и не снижаем Food `min_sources=2` ради красивого статуса. Не оставляем только env=`on` без permission-флагов, потому что это косметический режим без реального source skip.
- Ожидаемый эффект и метрика проверки: завтра `morning_inventory_intake_report.mode=on`; здоровые stable-категории появляются в `night_replaced_categories`, а неполные/degraded категории — в `live_fallback_categories` с точной причиной; `morning_consumed=true`; технический verify остаётся OK и выпуск отправляется. По состоянию 10.08 до production-run новый plan уже даёт `diaspora_events/professional_events=SKIP`; Culture=`current_night_scan_incomplete`, Tickets=`action_urls_not_verified`, Food=`post_contract_block_insufficient` и поэтому честно остаются live.
- Файлы/места: `.github/workflows/daily-digest.yml`; `collector/core.py:_MORNING_INVENTORY_DEFAULT_MODE`; `block_policy.py:BLOCK_POLICY_REGISTRY`; `tests/test_inventory.py:ReplacementPlanTest/NightWaveTest`.
- ПРОВЕРКА: полный набор `1006/1006` тестов OK. Replay 10.08: `38 shown / 0 replaced / 0 removed`, `5` content divergences → `ship_degraded`, lead `ok`, `blank_runs_2plus=0`, technical verify OK. Production safe-run `31372103724` на `a32f77e` завершился success за `15:24`, `mode=on`, `morning_consumed=true`: `4855 records_loaded → 175 merged_into_live + 2 inserted_into_pipeline`; утренний broad scan реально заменён ночью для `diaspora_events` и `professional_events`. Culture/Tickets/Food получили live-fallback соответственно по `current_night_scan_incomplete` / `action_urls_not_verified` / `post_contract_block_insufficient`; live/hybrid core-категории остались утренними по policy. Collector получил `1073` кандидата, dedupe и validation завершились, curator разобрал `119` включённых карточек, rank завершился success после circuit breaker DeepSeek и OpenAI fallback. Run был остановлен после rank: plan/send/warnings/state-commit пропущены, повторной отправки не было.
...
- ПРОВЕРКА: regressions `1 → fallback`, `3 → safe_to_skip`; существующие health/liveness/replacement tests OK. Полный suite `1018/1018` OK; replay 10.08 — `38 shown / 0 replaced / 0 removed`, technical verify OK. Production dry-runs `31380397745` и `31382681159` оставили `professional_events` в live collection с точной причиной `post_contract_backup_insufficient`; система при этом сохранила общий `mode=on`.
...
- ПРОВЕРКА: live-shaped triple даёт drop только MEN Wigan–Liverpool; существующий Eccles official/media regression также остаётся green. Полный suite `1018/1018` OK; replay 10.08 — `38 shown / 0 replaced / 0 removed`, technical verify OK. В production dry-run `31382681159` Wigan media duplicate отсутствовал в plan-execution и final judge; `duplicate_after_plan=0`, Wigan official transport не был снят как non-GM.
...
### 0228 — `on` действительно публикует проверенный ночной запас при source replacement — 2026-08-10
- Статус: ПРОВЕРЕНО-работает.
- Проблема: dry-run `31380397745` подтвердил `mode=on` и безопасно оставил Professional в live-fallback, но вскрыл логический разрыв для категории, которую ночь действительно заменила: `diaspora_events` прошёл health/floor/backup/liveness и morning broad scan был пропущен, тогда как actual intake по-прежнему удерживал самостоятельную ночную карточку без совпадающей morning-live карточки как `not_live_confirmed`. То есть решение «live можно не собирать» и правило «без live публиковать нельзя» могли выполняться одновременно.
- Причина (корень): `collector.core` передавал source-replacement plan только в список пропускаемых source categories. `build_morning_inventory_intake` о принятом решении не знал и разрешал вставку лишь после merge с live-кандидатом или HTTP 304 того же morning source. Для пропущенного источника получить такое подтверждение в том же run невозможно.
- Решение: actual intake получает точный набор категорий, для которых `inventory_source_replacement_plan.safe_to_skip=true`. Только в `mode=on`, только для этих категорий и только для operational row с собственным `action_url_liveness=alive` карточка получает подтверждение `current_night_replacement` и входит в обычный dedupe/validate/rank/plan. `assist`, hybrid-блоки, неполные category-wave, недостаточный floor/backup и unknown/dead URL сохраняют прежнее требование morning-live подтверждения. В funnel добавлен отдельный счётчик `confirmed_by_night_replacement`.
- Почему так: это замыкает уже существующий строгий контракт source replacement, не превращая stale inventory в публикацию и не добавляя второй обход dedupe/plan. Глобально отключить source skipping означало бы косметический `on`; безусловно доверять всем ночным строкам означало бы потерять health/liveness защиту.
- Ожидаемый эффект и метрика проверки: если категория появилась в `night_replaced_categories`, её подходящие unmatched ночные строки видны как `confirmed_by_night_replacement` и `inserted_into_pipeline`; если контракт не доказан, morning live collection выполняется как раньше. Финальный verify остаётся технически чистым.
- Файлы/места: `inventory.py:build_morning_inventory_intake`; `collector/core.py:collect_digest`; `tests/test_inventory.py:NightWaveTest`.
- ПРОВЕРКА: regression подтверждает прямую вставку alive operational card только при `mode=on + source_replacement_categories`; тот же вызов в `assist` остаётся `not_live_confirmed`. Inventory tests `78/78`; полный suite `1020/1020` OK. Replay 10.08: `38 shown / 0 replaced / 0 removed`, technical verify OK, lead `ok`, `blank_runs_2plus=0`. Production dry-run `31382681159` на `cd4c1e0` завершил полный путь за `25:35`: `mode=on`, `4855` records → `165` merged + `3` `confirmed_by_night_replacement` + `5` inserted; `diaspora_events` реально заменён ночью, Professional/Food/Tickets/Culture получили точные live-fallback. DeepSeek дал `23` HTTP 200 (`5` board + `16` rank-card batches + `2` rewrite batches), OpenAI card fallback=`0`; malformed JSON одного `last_24h` отклонён без circuit breaker. Plan `35` public slots + `18` backups, финал `32 shown / 0 replaced / 3 removed`, `unfilled=0`, `lines_outside_plan=0`, `empty_bullets=0`, `technical_errors=[]`, digest `15252` bytes. Telegram/post-publish/warnings/weekly/state-commit пропущены. Russian shortfall теперь объяснён не потерей intake, а честным `repeat_blocked:same_calendar_item_without_new_reader_moment` после того, как три ночные карточки действительно вошли в pipeline.
- Где была ошибка (если не сработает): сверить `source_replacement_plan[*].safe_to_skip`, actual-intake `funnel.confirmed_by_night_replacement`, candidate `inventory_live_confirmation` и индивидуальный `action_url_liveness`.

### 0229 — Final judge не редактирует обязательную подпись source-link — 2026-08-10
- Статус: внедрено; offline/replay-проверка завершена, production-proof ожидается после push.
- Проблема: production dry-run `31382681159` снял корректную карточку National Rail о восстановленном движении Liverpool–Wigan. Модель назвала `National Rail Enquiries` «лишним упоминанием в конце строки», хотя это была обязательная подпись HTML-ссылки на источник, а не редакционная проза.
- Причина (корень): `_digest_line_slots_from_html` превращал весь HTML в plain text вместе с видимым текстом `<a>…</a>` и отправлял source label судье как часть предложения. Repair executor затем честно пытался выполнить ложный patch; fact-lock не позволил потерять атрибуцию, запасного не было, и строка получила `fact_lock_failed`.
- Решение: source anchor удаляется только из editorial text payload map/reduce, но остаётся неизменным в HTML и URL identity. Prompt прямо запрещает actions против `source_link/source_label`. Перед repair добавлен детерминированный последний предохранитель: жалоба, которая называет точную подпись anchor и описывает её как source/link/текст «в конце строки», записывается в `source_link_metadata_rejected` и не меняет карточку. Реальная factual-жалоба на организацию в самой прозе этим фильтром не скрывается.
- Почему так: ссылка источника — технический контракт доставки; judge должен проверять текст новости, а не подпись транспорта. Полностью отключать format-проверки нельзя, поэтому guard требует одновременно точное совпадение label и metadata/end-of-line причину.
- Ожидаемый эффект и метрика проверки: точная жалоба из production run отклоняется до repair; National Rail карточка остаётся, а factual complaint про неверный статус National Rail продолжает выполняться.
...
- Статус: внедрено; production выборочно подтверждён, replay полностью пройден.
...
- ПРОВЕРКА: профильный набор 188/188, полный набор 1030/1030. Регрессии: Stockport Food vs office → 2/2 сохранены; All Points East Lorde vs Tyler → 2/2 сохранены; существующие same-story/premium тесты зелёные. Replay `--golden`: 12/12 дней завершены. Production dry-run `31788557384`: Food `7 collected → 2 included → 1 planned → 1 visible` (не ноль после прежнего ложного collapse); ticket canonical stamp сохранил `308` artist identities на `839` карточках.
- Где была ошибка (если не сработало): —

### 0231 — A-tier считается по артисту и сохраняет каждую физическую дату — 2026-08-14
- Статус: внедрено; production-proof пройден.
...
- ПРОВЕРКА: на живом пуле 14 series rows корректно разрешены (Lorde, The Cure, Biffy Clyro, Florence + The Machine, Tyler, the Creator, The Prodigy, Twenty One Pilots и др.). Replay 12.08: общий выпуск 40→41 карточка, «Крупные концерты вне GM» 5→6, plan shown 41→42, снятия остались 1, prose defects 0. Production dry-run `31788557384`: notability `839` карточек / `308` артистов, lookup `468`, classified `432`, coverage `0.923`; A-tier `29` физических событий / `14` артистов, eligible `22`, planned `22`, visible `22`, missing `[]`, conserved `true`.
- Где была ошибка (если не сработало): —

### 0232 — Ночной CV-вердикт переживает смену transport identity; Today не принимает завершённое действие — 2026-08-14
- Статус: внедрено; новая ночная CV-запись подтверждена в production, утренний reuse ожидается 15 августа.
...
- ПРОВЕРКА: одинаковые факты при двух fingerprint → hash equal; изменение venue → hash differs; completed court example → `(False, finished_action)`; профильный набор 188/188, полный 1030/1030. Production dry-run `31788557384`: CV contract `5 eligible → 5 sent`, outcomes `go=1 / consider=1 / skip=3`, `outcomes_conserved=true`, route failures `[]`. Новая production wave `31792957392`: `8 eligible → 8 sent → 4 applied / 4 skip`, outcomes `go=1 / consider=3 / skip=4`, conservation=true, route failures `[]`; inventory с новым evidence hash сохранён. Фактический `reused_night_verdict` проверяется утренним run 15 августа.
- Где была ошибка (если не сработало): —
...
- ПРОВЕРКА: до исправления production run `31782396441`: `mode=on`, три contract-rejection, `ranked_candidates=14`. После исправления run `31788557384`: `rank_stage=complete`, `ranked_candidates=62`, `board_rejected_responses=[]`; все DeepSeek rank-вызовы ответили HTTP 200, план `70` public slots; итог `64 shown / 4 replaced / 2 removed`, `technical_errors=[]`. Dry-run вывел полный block/section funnel, CV, notability и A-tier conservation.
...
### 0235 — Потери исполнения 14 августа устраняются до final judge — 2026-08-14
- Статус: внедрено; production-proof пяти исправленных классов пройден.
- Проблема: в реальном выпуске 14 августа ножевое ранение рядом со станцией ошибочно заняло transport-slot и было снято judge; две корректные recurring car-boot карточки были заменены после ложного требования вернуть прошлую дату JSON-LD; свежая rail-card снялась как `unsupported_fact`, потому что `13 августа 2026` читалось из `published_at`, но не входило в numeric fact-lock; неизменная Oldham Council карточка с тем же URL и evidence была показана уже пять раз под общей меткой `follow_up`. Отдельно новость о переносе офиса BT ложно считалась retail-opening только из-за слова `lease`.
- Причина (корень): transport reroute связывал любое упоминание поезда с любым `closed` в пределах всей карточки; date guard читал `event.date_start` раньше `next_occurrence`; numeric evidence исключал publication timestamp; repeat policy доверял `follow_up` для non-event без проверки сохранённого evidence; commercial predicate не требовал retail-контекста.
- Решение: sensitive incident направляется в transport только при явной связке mode+service impact; recurring date guard использует `next_occurrence/event_occurrence`; publication date добавлена в numeric evidence; exact fingerprint с неизменными title/published_at/evidence блокируется до generic follow-up (кроме operational/calendar классов); lease-rule требует реальный retail-контекст.
- Почему так: правила основаны на фактах и применяются до потери slot; корректные rail disruption, обновившийся same-URL сюжет, ticket/calendar milestone и ежедневный operational snapshot сохраняются.
- Ожидаемый эффект и метрика проверки: stabbing остаётся в hard news; recurring 16/15 августа помечаются `verified_existing_fact`; rail publication date не получает `unsupported_fact`; Oldham unchanged evidence получает `exact_fingerprint_unchanged_evidence`; BT не получает commercial/retail error.
- Файлы/места: `candidate_validator.py:_should_route_to_transport`; `pre_send_quality_judge.py:_line_has_expected_event_date`; `repeat_policy.py:_unchanged_exact_source_evidence/visible_repeat_verdict`; `writer.py:_number_evidence_tokens/_draft_line_quality_errors`; профильные regressions.
- ПРОВЕРКА: live-shaped probes подтвердили все пять исходов; affected tests `201/201`, полный suite `1036/1036` OK. Replay 14.08 до исправления: plan `66`, `63 shown / 1 replaced / 2 removed`; после: plan `63`, `63 shown / 0 replaced / 0 removed`. Production run `31788557384`: recurring New Smithfield 16 августа получил `verified_existing_fact`; rail/Today publication date также подтверждена без снятия; execution removal reasons содержат только `duplicate_after_plan=1` и `unrenderable_line=1`, без прежних `unsupported_fact`/`fact_lock_failed`; technical verify OK, `lines_outside_plan=0`, `empty_bullets=0`.
- Где была ошибка (если не сработало): сверить `section_routing_quality`, `event.next_occurrence/event_occurrence`, `governing_repeat_decision.reason`, writer numeric errors и `repair_executor.actions[*].method`.

### 0236 — Sensitive incident не становится transport-alert; geo backup обязан соответствовать разделу — 2026-08-14
- Статус: внедрено; offline/replay проверка пройдена, production-proof ожидается от следующего выпуска.
- Проблема: production dry-run `31788557384` показал ещё два класса поздней работы judge. Некролог после столкновения мотоцикла с замещающим автобусом попал в транспорт только по фразе `replacement bus`. Три Creamfields-карточки в Daresbury попали в GM-раздел «Дальние анонсы»; judge потребовал снять их за географию, но reserve-chain заменил их на Freshfields-карточки в том же Daresbury и назвал post-check успешным.
- Причина (корень): sensitive transport predicate принимал слово `replacement` за движение/сбой сервиса; `_apply_routing` безусловно переписывал любой far-future event в `future_announcements`, стирая `outside_gm_tickets`; reserve post-check проверял факты и полноту, но не venue scope раздела.
- Решение: sensitive incident требует явного утверждения о задержке/отмене/объезде/остановке сервиса либо конкретной rail-replacement схеме; far-future outside-GM ticket сохраняет outside lane и всё ещё обязан пройти A-tier; запасной event для GM-раздела отклоняется при declared/resolved outside/nearby venue scope.
- Почему так: правило сохраняет настоящие сообщения `bus services diverted` и `buses replace trains`, но не превращает сам факт участия автобуса в транспортную полезность. География проверяется и при планировании, и при позднем reserve, поэтому следующий non-GM venue закрыт тем же общим контрактом.
- Ожидаемый эффект и метрика проверки: fatal collision остаётся `last_24h`; реальный service disruption остаётся `transport`; Daresbury/London ticket не входит в `future_announcements`, а backup с той же географией фиксируется как rejected вместо ложного repair.
...
- ПРОВЕРКА: targeted `3/3`, весь `tests.test_inventory` `79/79`, полный набор `1041/1041`. Production-before run `31792959261`: все 40 сохранённых fact-ready ticket rows имели HTTP 403 и `action_url_liveness=unknown`. Production-after run `31793748393`: `82 checked → 80 reachable / 2 not_found / 0 unknown`; 40 сохранённых ready rows с HTTP 403 получили `action_url_liveness=alive`, 404 сохранились `unknown` и не были ложно оживлены.
...
- Статус: внедрено; production-proof пройден, night ticket replacement разрешён.
...
- ПРОВЕРКА: targeted `3/3`, полный набор `1042/1042`; integration test строит night inventory card без исходного `ticket_type` и получает `fact_card.ticket_type=event_this_week`. Production-before run `31793748393`: Manchester Academy `20/20` rows отсутствовали из ready только по `ticket_type`; replacement reason `action_urls_not_verified`. Production-after run `31794252959`: `838 found / 0 source errors / 143 fact-ready / 117 render-ready / 101 morning-eligible`; прирост ровно `+20` fact/render/morning rows. Preview на сохранённом inventory: Ticket Radar `25 candidates / 25 alive / 2 sources`, все floor/backup/liveness/health checks true, `venues_tickets.safe_to_skip=true`, reason `safe_post_contract_replacement`.
...
### 0239 — Weekend собирает полный типовой инвентарь и показывает его группами — 2026-08-20
- Статус: внедрено; funnel и четыре группы подтверждены production dry-run, последний межисточниковый дубль исправлен и ожидает контрольного production-proof.
- Проблема: 15–16 августа читатель получил примерно половину заметных рынков, фестивалей и временных выставок. В воскресенье публичный Weekend начинался с пяти car boot в одном сплошном списке; UK B-Boy Finals остался запасным, Ai Weiwei был снят без даты, а Sale/C-Street/Bolton makers, Mottram Show, Levenshulme Pride, GRUB, Reggae Brunch, IWM North и Wild Things не дошли до кандидатов. В субботу шесть разных рынков дополнительно схлопнулись текстовым дедупом.
- Причина (корень): Skiddle применял лимит к ближайшим событиям до приоритета текущего уикенда; в registry не было нескольких постоянных официальных календарей; полнота считалась только по числу карточек/источников и принимала пять однотипных car boot за достаточный выбор; curator сравнивал шаблонный текст разных рынков; planner оставлял защищённое Weekend-событие в reserve, если rank так решил; итоговый renderer не различал типы. Source coverage ошибочно засчитывал источнику любую карточку в любом разделе, а seller pitch fee мог стать ценой входа. Первый production dry-run дополнительно доказал, что generic MediaCity link parser терял диапазон дат с listing-card, а Ai Weiwei доверял перевёрнутому JSON-LD `2027-07-02 → 2026-09-06` вместо видимого диапазона страницы.
- Решение: Skiddle сначала сортирует текущий weekend, затем применяет cap 100/50; добавлены официальные MediaCity, Totally Stockport, Mottram Show, Levenshulme Pride, прямые Sale/Bolton Makers и Ai Weiwei и их явные tier-1 записи. Для MediaCity отдельный server-rendered card parser сохраняет title/category/start/end, borough=Salford, дочитывает venue с event-page и передаёт activity type; короткое название GRUB проходит source policy. Structured borough теперь имеет тот же приоритет, что остальные официальные event hints, поэтому локальные карточки не теряются только из-за отсутствия распознанной сущности в коротком venue. Direct event-page parser заменяет отсутствующий/перевёрнутый JSON-LD официальным видимым диапазоном. Weekend-контракт различает special events, exhibitions, one-off markets и routine markets/car boot; защищённые текущие события становятся primary даже после rank=reserve, curator не делает шаблонный semantic dedup внутри Weekend, а полнота требует минимум три семейства. Editor показывает четыре подзаголовка и предупреждает о концентрации, но не блокирует send. Деградированная night-wave больше не подавляет единственный scheduled fallback; coverage считает только рендер в Weekend; buyer-free имеет приоритет над seller fee. Final date QA сверяет recurring-карточку с сохранённым `next_occurrence`, включая поздний replay, а не возвращается к старой JSON-LD дате.
- Почему так: исправление сохраняет существующий slot-plan и детерминированные exact/event-identity dedup, не добавляет модель, стадию или очередь. Обычные walking tours, club listings и routine leisure не получают must-show только из-за полной event-карточки; все конкретные рынки/ярмарки/фестивали/временные выставки текущего уикенда сохраняются, а регулярные car boot лишь уходят в последний блок.
- Ожидаемый эффект и метрика проверки: после полной ночной волны Weekend не считается sufficient при шести карточках одного типа; текущие Skiddle events не теряются за distance cap; все eligible Weekend events находятся в primary slots; публичный раздел показывает `Фестивали и специальные события → Выставки → Ярмарки и рынки → Регулярные рынки и car boot`, а report хранит counts/dominant_share и section-specific source yield.
- Файлы/места: `data/sources.toml`; `collector/extract.py:_extract_skiddle_api_items`; `weekend_inventory.py:weekend_activity_group/is_weekend_inventory_candidate`; `inventory.py:inventory_block_completeness`; `curator.py:_BLOCKS_TO_SKIP_DEDUP`; `plan_digest.py:run_plan_digest`; `writer.py:_weekend_source_details/_weekend_activity_score/_collapse_weekend_duplicate_events`; `editor.py:_group_weekend_lines`; `release.py:_count_per_source_yield/_weekend_source_coverage_report`; `scripts/run_local_digest.py:_complete_inventory_wave_for_day`; `.github/workflows/daily-digest.yml:Report full dry-run result`; профильные tests.
- ПРОВЕРКА (offline/replay + first production audit): source probes до MediaCity parser: MediaCity `13`, Ai Weiwei `1`, Totally Stockport `30`, Levenshulme Pride `1` candidates; Mottram/Sale/Bolton pages fetched cleanly и дали `0` после current-date gate 20 августа, как ожидается для прошедшего 16 августа. Replay 16.08 до: plan `45`, writer/verify `44 shown`, `1 removed`, `43` bullets, Weekend `9`; после: plan `46`, writer/verify `45 shown`, `1 removed`, `44` bullets, Weekend `10`, B-Boy visible, composition `special=4 / exhibitions=1 / routine=5`, новых plan mismatch нет. Golden replay `12/12`: plan/write/verify counts совпали с baseline на каждом дне. Production night events run `32382626823`: `257 found / 75 fact-ready / 74 render-ready / 10 morning-eligible`, MediaCity `13→0 fact-ready`, Ai `1→1→0 eligible`; degraded status из-за одного чужого HTTP 500. Production dry-run `32383054526`: fallback `culture_weekly=current_night_scan_incomplete`, Weekend `11 planned=written=visible`, но composition `special=3 / markets=1 / routine=7`, что и открыло остаточный gap; technical verify true, Telegram skipped. После parser-fix реальные страницы дают GRUB=`markets`, Spies/Wild Things=`exhibitions`, Ai range=`2026-07-02→2026-09-06`; профильные `217/217`, полный набор `1052/1052`, replay 16.08 неизменен: `45 shown / 1 removed / 44 bullets`. Production night run `32388140121` подтвердил восстановление дат (`257 found / 77 fact-ready / 75 render-ready`), но также проявил последний барьер: короткие venues `Gardens`/`Lowry` не давали `gm_fit`; structured borough fix закрыт профильными `177/177` и полными `1053/1053` тестами, финальный production dry-run ожидается на актуальном commit.
- PRODUCTION-PROOF после structured borough fix: night run `32388863126` — `257 found / 79 fact-ready / 77 render-ready / 11 morning-eligible`; MediaCity `9 found / 3 fact-ready / 3 render-ready / 1 morning-eligible`, GRUB, Spies и Wild Things имеют status=`ready`, Ai Weiwei status=`ready`. Dry-run `32389257741` — live fallback `culture_weekly=current_night_scan_incomplete`, Weekend `217 collected → 49 included → 12 planned → 12 written → 12 visible`; всего `55 slots / 52 shown / 3 replaced / 0 removed`, `lines_outside_plan=0`, `empty_bullets=0`, technical verify=true, Telegram и state commit skipped. В dry-run отчёт добавлены `weekend_composition` и список реально показанных Weekend-кандидатов, чтобы последующие проверки формата были прямыми, а не косвенными.
- COMPOSITION-PROOF `32452314560`: Weekend `16 planned → 16 written → 16 visible`; `special_events=6 / exhibitions=2 / markets=1 / routine_markets=7`, все четыре группы, dominant share `0.438`; GRUB и Manchester Open видимы; technical verify=true, Telegram/state commit skipped. Список также выявил один межисточниковый дубль Magic of Thailand Festival: оба источника дали одинаковые дату/событие, но один venue был коротким, другой содержал тот же venue с адресом. Writer теперь считает такие venue совместимыми по containment, а при отсутствующем venue — по точному набору смысловых токенов+дате; два одинаково названных события на разных явных площадках сохраняются. Профильные `18/18`; replay 16.08 до/после неизменен: `45 shown / 1 removed / 44 bullets`.
- REGRESSION-PROOF последнего dedup-fix: полный набор `1055/1055`; старый time-of-day-зависимый transport-тест зафиксирован на 08:31 London и больше не зависит от времени запуска suite.
- Где была ошибка (если не сработало): проверить `collector_report.source_health`, Skiddle `raw_extracted→candidates`, `night_supply_completeness.activity_type_count`, curator `semantic_dropped`, `release_plan` primary/reserve, `editor_report.weekend_composition`, `release_report.source_status.weekend_rendered_count` и запуск fallback после degraded wave.
